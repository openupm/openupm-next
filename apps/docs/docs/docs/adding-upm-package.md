...
For monorepos or repositories with duplicate tag families, set `gitTagPrefix` to the literal prefix that identifies this package's tags, such as `upm/`, `com.example.package/`, or `com.example.package-`. This prevents tags for another package or branch family from queueing builds for the wrong package. The prefix is only a literal filter, not a regular expression, and is not stripped before version parsing. For example, `gitTagPrefix: "com.example.package/"` accepts `com.example.package/1.2.3-preview.1` as version `1.2.3-preview.1`.
...
Release does not exist yet, or exists but does not yet contain a publishable
